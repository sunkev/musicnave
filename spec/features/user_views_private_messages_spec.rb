...
feature 'view all private messages' do
  ...
  scenario "user sees private messages" do
    ...
    select adam.name_and_username, from: 'private_message_recipient_id'
    fill_in "Body", with: body_paragraph
    click_button "Send message"

    click_link 'View messages'
    expect(page).to have_content(adam.name_and_username)
    expect(page).to have_content(kevin.name_and_username)
